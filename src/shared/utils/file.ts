...
export function parseMultipartFile(body: Buffer, contentType?: string): {
  originalName: string;
  storedName: string;
  mimeType: string;
  buffer: Buffer;
...
export function isValidProjectFile(mimeType: string, extension: string): boolean {
  const isAllowedMimeType = ALLOWED_IMAGE_MIME_TYPES.has(mimeType) || ALLOWED_DOCUMENT_MIME_TYPES.has(mimeType);
  const isAllowedExtension = ALLOWED_IMAGE_EXTENSIONS.has(extension) || ALLOWED_DOCUMENT_EXTENSIONS.has(extension);
  return isAllowedMimeType && isAllowedExtension;
}

export function isValidPaymentFile(mimeType: string, extension: string): boolean {
  const isAllowedMimeType = ALLOWED_IMAGE_MIME_TYPES.has(mimeType);
