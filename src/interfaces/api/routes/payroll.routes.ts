...
export function createPayrollRoutes(payrollRepo: SqlitePayrollRepository): Router {
  ...
  router.get('/workers', async (_req: Request, res: Response) => {
    try {
      ...
      logger.error('Failed to list workers', { error: error.message });
  ...
  router.post('/workers', async (req: Request, res: Response) => {
    try {
      ...
      logger.info('Worker created', { id: worker.id, name: worker.name });
      res.status(201).json(worker);
    } catch (error: any) {
      logger.error('Failed to create worker', { error: error.message, body: req.body });
  ...
  router.patch('/workers/:id', async (req: Request, res: Response) => {
    try {
      ...
      if (!current) return res.status(404).json({ error: 'Worker not found' });
      ...
      logger.error('Failed to update worker', { error: error.message, id: req.params.id, body: req.body });
      res.status(400).json({ error: error.message });
    }
  });

  router.patch('/workers/:id/status', async (req: Request, res: Response) => {
    try {
      const current = await payrollRepo.findWorkerById(req.params.id);
      if (!current) return res.status(404).json({ error: 'Worker not found' });
      ...
      logger.error('Failed to update worker status', { error: error.message, id: req.params.id });
      res.status(400).json({ error: error.message });
    }
  });

  router.put('/workers/:id/months/:period', async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Failed to set worker salary month', { error: error.message, id: req.params.id, body: req.body });
      res.status(400).json({ error: error.message });
    }
  });

  router.post('/workers/:id/advances', async (req: Request, res: Response) => {
    try {
      const worker = await payrollRepo.findWorkerById(req.params.id);
      if (!worker) return res.status(404).json({ error: 'Worker not found' });

      const paidAt = req.body.paidAt ? new Date(req.body.paidAt) : new Date();
      if (Number.isNaN(paidAt.getTime())) return res.status(400).json({ error: 'Paid date is invalid' });
      ...
      logger.error('Failed to create worker advance', { error: error.message, id: req.params.id, body: req.body });
      res.status(400).json({ error: error.message });
    }
  });
  ...
  router.delete('/advances/:id', async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Failed to delete worker advance', { error: error.message, id: req.params.id });
      res.status(400).json({ error: error.message });
    }
  });
  ...
  router.get('/summary', async (req: Request, res: Response) => {
    try {
      ...
      const advancesByWorker = new Map<string, typeof advances>();
      ...
      logger.error('Failed to build payroll summary', { error: error.message, period: req.query.period });
      res.status(400).json({ error: error.message });
    }
  });
